...
把“小红书图文草稿执行层”先做成稳定的本地桥接能力，并已经内收到 OpenClaw 仓内。
...
- 优先复用已有的 `https://creator.xiaohongshu.com/...` 创作者标签页
- 只有完全没有创作者页面时才新开
...
6. 专用浏览器
   - 浏览器用户目录：`~/.codex/chrome-mcp-profile`
   - 远程调试端口：`9222`
...
已发布的冒烟验证测试帖现在可通过 `cleanup-published` 命令按精确标题或 `smoke-report.json` 清理，但草稿箱仍只有“编辑最新笔记”入口，没有独立草稿列表页，因此草稿清理暂时仍需人工执行。
...
- 本轮冒烟验证已确认仓内内收后的执行层可以直接工作，不再依赖 `../xiaohongshu-mcp`
- 本轮为了避免真实发帖，没有执行 `click-publish` / `publish` 的线上动作
- 本轮 `draft` 会在草稿箱新增一条测试草稿，标题为 `OpenClaw XHS smoke 2026-03-07 14:51:12`

## 2026-03-07 扩展冒烟验证与加固结果

- 已新增 `pnpm xhs:smoke`
- 已把 `check-login` 的标签页复用校验纳入冒烟验证
- 已把 `draft` 的真实填充与存草稿校验纳入冒烟验证
- 已把“离开编辑页后 `save-draft` 必须失败”纳入冒烟验证
- 已为真实发帖验证补上显式保护开关，避免误发
- 已加固发布页的关键页面结构选择器与按钮识别逻辑
...
5. 已补 `pnpm xhs:smoke`，并完成真实非发布冒烟验证
6. click-publish 和 publish 两条路径都已完成真实线上验证
7. 已修复频繁新开小红书页面的问题，现在默认复用同一个创作者标签页
8. `fill` / `draft` / `publish` 已支持 `--audit-dir` 输出 `preview.png` 与 `summary.json`
9. `pnpm xhs:smoke` 已支持 `--report-dir` 输出 `smoke-report.json` 与 `cleanupNotes`
10. 已支持 `cleanup-published`，对已发布冒烟验证测试帖做预演或正式清理

当前最大问题：
当前执行层已闭环，但仍依赖页面结构选择器，草稿清理仍需人工处理，也还没有 CI 级无人值守验收。
